...
sudo pip3 install ffmpeg-python google-api-python-client
...
sudo sed -i '/\b\(function camera.timelapse.upload\)\b/d' ~/.bash_aliases
sudo sed -i '$ a function camera.timelapse.upload { sudo python3 ~/camera.timelapse/camera.timelapse.upload.py $@; }' ~/.bash_aliases
